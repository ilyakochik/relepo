...
class Player_v7(BasePokerPlayer):
    ...
    def __init__(self, batch=10 ** 3, memory=10 ** 5, alpha=0.01, gamma=0.8,
                 epsilon=0.05, hidden_layers=(21,), name=None):
        super().__init__()
        ...
        inputs = [(2, 52), (52,), (52,), (52,)]
        self._w_init(inputs, hidden_layers)
        self._w_compile()

    def _w_init(self, inputs, hidden_layers):
        # create separate inputs
        inp_l = []
        hid_l = []
        for k, v in enumerate(inputs):
            k_inp_l = tf.keras.Input(shape=v, name=f'input_{k}')
            k_hid_l = tf.keras.layers.Flatten(input_shape=v, name=f'flatten_i_{k}')(k_inp_l)
            k_hid_l = tf.keras.layers.Dense(k_hid_l.shape[1], activation='relu', name=f'dense_i_{k}')(k_hid_l)

            inp_l.append(k_inp_l)
            hid_l.append(k_hid_l)

            self._D_states.append(np.full((self._memory,) + v, False))

        # merge inputs if more then 1
        if len(inputs) > 1:
            hid_l = tf.keras.layers.Concatenate()(hid_l)
        else:
            hid_l = hid_l[0]

        # add hidden_layers
        for i in hidden_layers:
            hid_l = tf.keras.layers.Dense(i, activation='relu')(hid_l)

        # add outputs
        hid_l = tf.keras.layers.Dense(3)(hid_l)
        out_l = tf.keras.layers.Softmax()(hid_l)

        self._w = tf.keras.Model(inputs=inp_l, outputs=out_l)
    ...
    def _action_random(self, valid_actions):
        ...
        amount = self._amount_random(valid_actions[action_id]['amount'])
    ...
    def _action(self, valid_actions):
        ...
        if action not in valid_actions_:
            ...
            amount = self._amount_random(valid_actions_[action])

            log.debug('{}: playing softmax {} {}'.format(self.name, action, amount))

            return action, amount

    def _amount_random(self, valid_action):
        if not isinstance(valid_action, dict):
            return valid_action
        else:
            return round(np.random.uniform(low=valid_action['min'], high=valid_action['max']), -1)

    def _get_P(self, state):
        """ Policy-values """
        for i, v in enumerate(state):
            state[i] = state[i][np.newaxis]
        P = self._w(state).numpy()[0]
    ...
    def _learn(self):
        ...
        for i, v in enumerate(self._history_states):
            ...
            for k, v in enumerate(state):
                self._D_states[k][self._D_i] = state[k]
            ...
            if (self._D_i + 1) % self._batch == 0:
                ...
                self._w.train_on_batch(
                    x=[x[ids_batch] for x in self._D_states],
    ...
    def _calc_state(self, round_state):
        ...
        # Hole cards
        sparse = np.array([
            np.isin(self._state_ids, self._hole_card[0]),
            np.isin(self._state_ids, self._hole_card[1])
        ])
        state.append(sparse)

        # Community cards
        for k in range(3):
            if k < len(round_state['community_card']):
                sparse = np.isin(self._state_ids, round_state['community_card'][k])
            else:
                sparse = np.full(52, False)
            state.append(sparse)
        P = self._get_P(state)
    ...
    def _w_to_str(self):
        ...
        for i, v in enumerate(self._w.layers):
            ...
            if layer == 'Dense':
                ...
                layer_str = '{} ({})\t\tparams\t{}\t(top-5={:.2f}, 0.1 top > {:4d}, 0.01 top > {:3d})'.format(
                    v.name,
                    v.units,
                    v.count_params(),
                    max_weights,
    ...
    def __setstate__(self, state):
        ...
        self._w = tf.keras.Model.from_config(config)
