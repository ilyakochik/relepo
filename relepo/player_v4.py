...
class Player_v4(BasePokerPlayer):
    """ End-of episode MC-alpha control using Q-function approximation
    via NN (hole cards -> 3 discreet actions) """

    def __init__(self, batch=1000, memory=10 ** 5, alpha=0.1, gamma=0.8, epsilon=0.05, name=None):
        super().__init__()
        ...
        self._w = tf.keras.models.Sequential([
            tf.keras.layers.Flatten(input_shape=(2, 52)),
        ...
        self._batch = batch
        self._memory = memory
        self._D_states = np.zeros(shape=(memory, 2, 52), dtype=bool)
    ...
    def _calc_state(self, round_state):
        ...
        state = [[i1 == i2 for i1 in self._state_ids] for i2 in self._hole_card]
        Q = self._get_Q(state)
    ...
    def _w_to_str(self):
        ...
        for i, v in enumerate(self._w.layers):
            ...
            params = v.count_params()
            str = f'Layer {layer}, {params} params'
            if layer == 'Dense':
                weights = v.get_weights()
                weights = np.sort(np.abs(weights[0].flatten()))
                max_weights = np.mean(weights[-5:])
                str += ' (top-5={:.2f}, 0.1 top > {}, 0.01 top > {})'.format(
                    max_weights, (weights < max_weights / 10).sum(), (weights < max_weights / 100).sum())
            ret.append(str)
        return '\n'.join(ret)
